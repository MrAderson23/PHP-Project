	...
	if($_POST['update']){
		...
		if(($_POST['agree'] != NULL) || ($fileSize > 0)){		
			...
			if (empty($errors)) {
				...
				$didUpload = move_uploaded_file($fileTmpName, "./img/pics/" .$name.".".$fileExtension);
			}
...
<head>
    ...
    <meta charset="utf-8">
    <title> Atom's Organizer | Search </title>
</head>
...
<body>
        ...
        <div class="collapse navbar-collapse" id="navbarNavAltMarkup">
            <div class="navbar-nav">
                ...
                <form method="post" action="search.php">
                    <input style="font-size: 100%!important; position: relative!important; margin: auto!important; width: 8.3rem!important;" type="text" name="search" placeholder="Search..." required="">
	...
	<div ".$drag."id='%s' class='item template-upload'>
	<div id='edtdata%s'>
	<img class='pic' align='center' src='%s'></img><br>
	<a>FIRST NAME: %s</a><br>
	<a>LAST NAME: %s</a><br>
	<a>EMAIL: %s</a><br>
	<a>ADDRESS: %s</a><br>
    ...
    <a>PHONE: %s</a><br>
	        	...
	        	<a style='cursor: pointer;' id='test' onclick='toggle%s(); togg%s()'>   <svg fill='#fff' xmlns='http://www.w3.org/2000/svg' xmlns:xlink='http://www.w3.org/1999/xlink' fill='#000000' version='1.1' id='Capa_1' width='32px' height='32px' viewBox='0 0 494.936 494.936' xml:space='preserve'>
<g>
	<g>
		...
		<path d='M483.876,20.791c-14.72-14.72-38.669-14.714-53.377,0L221.352,229.944c-0.28,0.28-3.434,3.559-4.251,5.396l-28.963,65.069    c-2.057,4.619-1.056,10.027,2.521,13.6c2.337,2.336,5.461,3.576,8.639,3.576c1.675,0,3.362-0.346,4.96-1.057l65.07-28.963    c1.83-0.815,5.114-3.97,5.396-4.25L483.876,74.169c7.131-7.131,11.06-16.61,11.06-26.692    C494.936,37.396,491.007,27.915,483.876,20.791z M466.61,56.897L257.457,266.05c-0.035,0.036-0.055,0.078-0.089,0.107    l-33.989,15.131L238.51,247.3c0.03-0.036,0.071-0.055,0.107-0.09L447.765,38.058c5.038-5.039,13.819-5.033,18.846,0.005    c2.518,2.51,3.905,5.855,3.905,9.414C470.516,51.036,469.127,54.38,466.61,56.897z'/>
       	...
       	<a id='test' href='delete_member.php?id=%s' onclick='return confirm();'> <svg fill='#fff' version='1.1' id='Capa_1' xmlns='http://www.w3.org/2000/svg' xmlns:xlink='http://www.w3.org/1999/xlink' width='32' height='32' x='0px' y='0px' viewBox='0 0 729.837 729.838' style='enable-background:new 0 0 729.837 729.838;' xml:space='preserve'>
<g>
	<g>
		<g>
			<path d='M589.193,222.04c0-6.296,5.106-11.404,11.402-11.404S612,215.767,612,222.04v437.476c0,19.314-7.936,36.896-20.67,49.653
...
<g>
    ...
    <form action='' method='post' enctype='multipart/form-data'  id='edt%s' hidden='hidden'>
      
      <input placeholder='ID' type='text' name='id' id='id' readonly value='%s'>
	  
     
      <input placeholder='First name' type='text' name='firstname' id='firstname' value='%s'>
      
      
      <input placeholder='Last name' type='text' name='lastname' id='lastname' value='%s'>
      
      
      <input placeholder='Email' type='text' name='email' id='email' value='%s'>
      
      
      <input placeholder='Address' type='text' name='address' id='address' value='%s'>
      
      
      <input placeholder='City' type='text' name='city' id='city' value='%s'>
      
      
      <input placeholder='Country' type='text' name='country' id='country' value='%s'>
      
      
      <input placeholder='Phone' type='text' name='phone' id='phone' value='%s'>
	  
	  
	  <input placeholder='New password' type='password' name='password' id='password' value=''>
	  <input style='display: none;' name='pic' value='%s'>
      <br></br>
	  <img class='pic' align='center' src='%s'></img><br>
  ...
  let toggle%s = () => {
    ...
    let element = document.getElementById('edt%s');
    ...
    let element = document.getElementById('edtdata%s');
    let hidden = element.getAttribute('hidden');
	...
	",$r["id"],$r["id"], $r["pic"], $r["firstname"],$r["lastname"], $r["email"],$r["address"], $r["city"], $r["country"], $r["phone"],$r["id"], $r["id"], $r["id"], $r["id"], $r["id"],$r["firstname"],$r["lastname"], $r["email"], $r["address"], $r["city"], $r["country"], $r["phone"], $r["pic"],$r["pic"] ,$r["id"], $r["id"], $r["id"], $r["id"]);
      }} else { echo "<a style='border: 0.5px solid rgba(0, 0, 0, 0.2); width: 30%; background-color: rgb(248 249 250 / 44%)!important; backdrop-filter: blur(60px) saturate(210%); border-radius: 17px; top: 2PX;  margin: auto; margin-bottom: 10px;box-shadow: inset 0 0 0 1px rgb(255 255 255 / 20%), 0 8px 40px rgb(0 0 0 / 25%);'>No results found</a>"; }
    }
    ?>
